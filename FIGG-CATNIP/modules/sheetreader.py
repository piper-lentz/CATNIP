...
def global_setup(wb, settings_sheet):
    ...
    if 'Input' in settings_dict and 'Value' in settings_dict:
        for i, key in enumerate(settings_dict['Input']):
            if key == '':
                continue
            defaults[key] = settings_dict['Value'][i]

    if 'Subdirectory' in settings_dict and 'Specs' in settings_dict:
        for i, subdir in enumerate(settings_dict['Subdirectory']):
            if subdir == '':
                continue
            ...
            for spec in settings_dict['Specs']:
                if spec == '':
                    continue
                try:
                    specs[subdir][spec] = settings_dict[spec][i]
                except Exception:
                    print(f"Missing column {spec} in settings sheet")
...
def addpaths(wb, im_data, namekey, settings_sheet):
    ...
    if settings_sheet is None:
        if 'File Name' in im_data:
            ...
            raise KeyError("Missing File Name")

    else:
        if im_data.get('Subdirectory Override'):
            filedir = im_data['Subdirectory Override']
        else:
            lvls = settings_sheet['Hierarchy']
            filedir = ''

            for lvl in lvls:
                if im_data.get(lvl) is None:
                    continue
                filedir += im_data[lvl] + '/'
        ...
        if 'Path' not in im_data:
            raise FileNotFoundError(f"No file found for {im_data[namekey]}")
...
def get_subdir(mockup, index, settings_sheet):
    ...
    for lvl in settings_sheet['Hierarchy']:
        try:
            subdir += mockup[lvl][index] + '/'
        except KeyError:
            print(f"Missing hierarchy level {lvl}")
    return subdir
...
def wb_to_dict(wb, sheet, add_paths=False, settings_sheet=None,
    ...
    for index, objname in enumerate(mockup[namekey]):
        ...
        for key in all_keys:
            ...
            # 1. Disk Information (highest priority)
            if key in mockup and mockup[key][index] != '':
                value = mockup[key][index]

            # 2. Disk Settings defaults
            elif key in defaults:
                value = defaults[key]

            # 3. Subdirectory overrides
            elif settings_sheet is not None and 'Hierarchy' in settings:
                subdir = (
                    get_subdir(mockup, index, settings)
                    if mockup.get('Subdirectory Override', [''])[index] == ''
                    else mockup['Subdirectory Override'][index]
                )

                if subdir in specs and key in specs[subdir]:
                    value = specs[subdir][key]
                else:
                    value = ''
            ...
            image[key] = clean_value(value)

        if add_paths:
            addpaths(wb, image, namekey, settings)

        if splitkey is None:
            images[objname] = image
        else:
            images[image[splitkey]][objname] = image
