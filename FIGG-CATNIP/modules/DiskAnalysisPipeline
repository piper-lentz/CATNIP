'''
DiskAnalysisPipeline
This module uses DiskImageProcessor module to process your disk images, uses 
DiskProfileExtractor to generate profiles, then creates the final 
multipanel figure.



CATNIP: Comparative Analysis Tool for Normalized Imagery and Profiles.
Processes multiwavelength disk images using DiskImageProcessor, and computes radial
and azimuthal profiles according to user input.
Original written by Cat Sarosi, 2022
Modified by Bibi Hanselman
Further modified by Piper Lentz, 2026
...
the DiskImageProcessor module during initialization to process the images according to
user-defined parameters.
Revised even further by Piper Lentz (2026). Refactored code, added 
ValueError messages, debugging (ex: you can plot one rad profile 
instead of needing to plot 2)
'''
from DiskProfileExtractor import AstroImage
...
class AstroObject():
    ...
    def __init__(self, name, image_dict, display_dict):
        ...
        self.prc_ims, self.prc_scales = DiskImageProcessor.process(image_dict, name = self.name, custom_ims=self.img_data_dict)#PL 
...
#############################################################################
    ...
    def update(self, display_dict):
        ...
        # PL added so that all PA inputs are used as a list throughout the code
        # without this, CATNIP can't plot just 1 radial profile
        pa_input = display_dict['PA List']
        if pa_input is None:
            self.PAlist = []
        elif isinstance(pa_input, list):
            self.PAlist = pa_input
        else:
            self.PAlist = [pa_input]
...
#############################################################################
    ...
    def plot_im(self, im, im_ax, subfig):
        ...
        '''
        ### define plotting bounds with sel.boundsize, which is determined by [Preview Crop]
        bounds = {}
        ...
        # renames line images to have a more descriptive name
        ...
        # ANNOTATIONS 
        # if Preview (in sheet), view the image with = radius circles
        # if not Preview, make images with az rings and rad lines to be shown with profiles
        if self.preview_radstep is None:
            raise ValueError('Provide a Preview Radstep value in the google sheet') # PL added
        elif self.preview:
        ...
        if self.au_scale != 0:
            ...
            if boundsize == 0: # PL added for value error
                raise ValueError("Preview Crop is too small — leads to zero-sized bounds.") # PL added
    ...
    def make_plot_multiline(self):
        ...
        for index, plot in enumerate(self.plot_types):
            ...
            suptitles = {'avgrad': 'Azimuthally Averaged Radial Profiles',
                         'rad': 'Radial Profiles',#'rad': 'Radial Profiles at Given PAs',
                         'az': 'Azimuthal Profiles',
                         'rad_az': 'Polar Projections',
                         'deprj_ims': 'Deprojected Imagery',
                         'or_ims': 'On Sky'}
            ...
            if 'ims' in plot:
                ...
                # Make subplots
                if self.n_heatmaps==0: # PL added
                    raise ValueError('Set Plot Heatmap equal to True') # PL added - something wonk with not having this True
        ...
        for index, im in enumerate(self.imlist):
            ...
            if 'rad' in self.plot_types:
                for i in range(self.max_PAlist_len):
                    rad_ax = axs['rad'][i]
                    rad_data = im.rad_plot_data[im.rad_plot_data['PA'] == int(self.PAlist[i])] # and it used to be == int(...)
